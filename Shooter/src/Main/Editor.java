...
//		g.setBackground(Color.black);
		...
		turret.draw(200, 200);
		turret.setCenterOfRotation(30/2, turret.getHeight()/2);
		
		Rectangle box = new Rectangle(200 + 30/2, 200 + turret.getHeight()/2, 1, 1);
		g.draw(box);
		
		float xDistance = (mx) - 200;
		float yDistance = 200 - (my);
		float theta = (float) Math.toDegrees(Math.atan2(xDistance, yDistance));

		g.drawString("" + turret.getRotation(), 100, 100);
		
		turret.setRotation(theta - 90);
		
		Line line = new Line(200 + 15, 200 + 15, mx, my);
		g.draw(line);
		
		float rad = (float) (theta * (Math.PI / 180));
		
		float endX   = (float) (Math.sin(rad));
		float endY   = (float) (-Math.cos(rad)) - 5;
		
		
		int distance = 32; //25 pixels

		double angle = Math.atan2(-yDistance, xDistance) * 180/Math.PI;

		float xSpawn = (float) (211 + Math.cos(angle * Math.PI/180) * distance);
		float ySpawn = (float) (211 + Math.sin(angle * Math.PI/180) * distance);
		
		Gun.recoil.setRotation(theta - 90);
		Gun.recoil.draw(xSpawn, ySpawn);
...
//		font.drawString(10, 10, "Size: " + size, Color.red);
//		int disX = sx/size;
//		int disY = sy/size;
//		if(disX < 0) disX = 0;
//		if(disX > 50) disX = 50;
//		if(disY < 0) disY = 0;
//		if(disY > 50) disY = 50;
//		font.drawString(10, 25, "X: " + disX, Color.red);
//		font.drawString(10, 40, "Y: " + disY, Color.red);
//		
...
//		
//		
//		font.drawString(10, 100, "Legend:", Color.red);
//		font.drawString(10, 120, "[C] Clear.", Color.red);
//		font.drawString(10, 140, "[F] Fill.", Color.red);
//		font.drawString(10, 160, "[Space] Change map drag.", Color.red);
//		font.drawString(10, 180, "[ENTER] Send map to server.", Color.red);
//		font.drawString(10, 200, "[Right Click] Add unblocked tile.", Color.red);
//		font.drawString(10, 220, "[Left Click] Add blocked tile.", Color.red);
//		font.drawString(10, 240, "[Back] Map selection.", Color.red);
//		font.drawString(10, 260, "[S] Save map.", Color.red);
//		
//		
